...
gf_point(log(Age) ~ Weight, data = TigerModel, color = ~ Sex)%>%
  gf_line(fits2~Weight)
```

** Questions**

* Create a third model with corresponding graphs, $Tiger2.lm = lm(Age ~ Weight + Sex, data = Tiger1)$
...
* Look at the residual plots. Is a data transformation useful?  
* Based upon the six models you created, which would you suggest using? Why?
