...
```{r}
Tiger1 <- read_csv("data/TigerData2.csv")   # Reads the data 
head(Tiger1, 5)   
```
...
Is there an important interaction term between Sex and any of the size variables when predicting age?

What is the best predictor of Age? 

Use a simple linear regression model to predict Age based upon NoseBlack

Look at the residual plots. Is a data transformation useful.  [In fact this type of data is almost always slightly improved by the arcsine transformation. It is very difficulty to see in the smaller Tiger1 sample, but is a little more obvious with the larger Tiger2 sample.You see it if you collect lots of older tigers, but they are rare in the population.]

Is there a benefit to adding additional terms to the model?
[If you want to predict size, Sex may be important, but most additional terms only slightly improve the model]
