...
We start by reading in the data and requiring the packages that we will need.
...
```{r SLR, exercise=TRUE, exercise.eval=TRUE}
...
```{r SLR2, exercise=TRUE, exercise.eval=TRUE}
...
* Since the slope fo the regression line is small ( - 0.213) can we conclude it is unimportant?
...
## Finding specific values in our regression model

Notice that each term in the linear model has been assigned a value:

```{r SLR3, exercise=TRUE, exercise.eval=TRUE}
str(summary(Cars.lm))
```

We can find the coefficents of our model with the `coef(Cars.lm)`, but for other terms we use the `$` symbol to pull any specific values. For example:

```{r SLR4, exercise=TRUE, exercise.eval=TRUE}
summary(Cars.lm)$r.squared
```



It is often useful to add the fitted values and the residuals to our original data

```{r SLR5, exercise=TRUE, exercise.eval=TRUE}
GMCars1 = mutate(GMCars1, res1 = Cars.lm$res, fits1 = Cars.lm$fitted.values)
```
...
```{r SLR6, exercise=TRUE, exercise.eval=TRUE}
gf_histogram(~ res1, data = GMCars1)
```


```{r SLR7, exercise=TRUE, exercise.eval=TRUE}
...
```{r MRM, exercise=TRUE, exercise.eval=TRUE}
Cars.lm2 = lm(Price ~ Mileage + Make, data = GMCars1)  # Creating a simple linear model
```


```{r MRM2, exercise=TRUE, exercise.eval = TRUE}
GMCars1 = mutate(GMCars1, res2 = Cars.lm2$res, fits2 = Cars.lm2$fitted.values)
...
```{r MRM3, exercise=TRUE, exercise.eval=TRUE}
summary(Cars.lm)$r.squared

summary(Cars.lm2)$r.squared

summary(Cars.lm2)$adj.r.squared

```

```{r MRM4, exercise=TRUE, exercise.eval=TRUE}
GMCars1 = mutate(GMCars1, res2 = Cars.lm2$res, fits2 = Cars.lm2$fitted.values)
...
```{r MRM5, exercise=TRUE, exercise.eval=TRUE}
GMCars1 = mutate(GMCars1, res2 = Cars.lm2$res, fits2 = Cars.lm2$fitted.values)

gf_point(res2 ~ fits2, data = GMCars1)%>%
  gf_hline(yintercept = 0)

gf_point(res2 ~ Make, data = GMCars1)%>%
  gf_hline(yintercept = 0)
```
...
Notice that simply adding the `Make` variable intro our model forces the slope to be identicle in all three cases. However, a closer look at the data indicates that their may be an **interaction effect** since the effect of `Mileage` on `Price` appears to depend upon the `Make` of the car. To include interaction terms in our model, we use $X_1*X_2$
...
```{r inter2, exercise=TRUE, exercise.eval = TRUE}
GMCars1 = mutate(GMCars1, res3 = Cars.lm3$res, fits3 = Cars.lm3$fitted.values)
...
```{r inter3, exercise=TRUE, exercise.eval=TRUE}

summary(Cars.lm2)$adj.r.squared

summary(Cars.lm3)$adj.r.squared

```

```{r inter4, exercise=TRUE, exercise.eval=TRUE}
GMCars1 = mutate(GMCars1, res3 = Cars.lm3$res, fits3 = Cars.lm3$fitted.values)
...
```{r inter5, exercise=TRUE, exercise.eval=TRUE}
GMCars1 = mutate(GMCars1, res3 = Cars.lm3$res, fits3 = Cars.lm3$fitted.values)

gf_point(res3 ~ fits3, data = GMCars1)%>%
  gf_hline(yintercept = 0)

gf_point(res3 ~ Make, data = GMCars1)%>%
  gf_hline(yintercept = 0)
```


After plotting a few more residual plots, try creating a model that you think would be a better fit.
...
```{r full1, exercise=TRUE, exercise.eval=TRUE}
...
Automoated multivariate regression procedures (the process of using prespecified conditions to automatically add or delete
...
```{r corr1, exercise=TRUE, exercise.eval=TRUE}
...
```{r corr2, exercise=TRUE, exercise.eval=TRUE}
model2 = lm(Price ~ Mileage + Cyl, data = GMCars2)  # Creating a simple linear model
anova(model2)
```

```{r corr3, exercise=TRUE, exercise.eval=TRUE}
...
**In multiple regression, the p-values for individual terms are highly unreliable and should not be used to test for the importanct of a variable** 
